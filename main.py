...
def main():
    text_pairs, train_pairs, val_pairs, test_pairs = load_data()
    ...
    eng_vocab = train_word_piece(text_samples=eng_samples,
                                 vocab_size=ENG_VOCAB_SIZE,
                                 reserved_tokens=reserved_tokens)

    spa_samples = [text_pair[1] for text_pair in train_pairs]
    spa_vocab = train_word_piece(text_samples=spa_samples,
                                 vocab_size=SPA_VOCAB_SIZE,
                                 reserved_tokens=reserved_tokens)
    ...
    train_ds = make_dataset(pairs=train_pairs,
                            eng_tokenizer=eng_tokenizer,
                            spa_tokenizer=spa_tokenizer,
                            max_sequence_length=MAX_SEQUENCE_LENGTH,
                            batch_size=BATCH_SIZE)
    val_ds = make_dataset(pairs=val_pairs,
                          eng_tokenizer=eng_tokenizer,
                          spa_tokenizer=spa_tokenizer,
                          max_sequence_length=MAX_SEQUENCE_LENGTH,
                          batch_size=BATCH_SIZE)
    ...
    for i in range(2):
        ...
        translated = decode_sequences(input_sentences=tf.constant([input_sentence]),
                                      eng_tokenizer=eng_tokenizer,
                                      spa_tokenizer=spa_tokenizer,
                                      max_sequence_length=MAX_SEQUENCE_LENGTH,
    ...
    ** Example 9 **
    ...
    for test_pair in test_pairs[:30]:
        ...
        translated_sentence = decode_sequences(input_sentences=tf.constant([input_sentence]),
                                               eng_tokenizer=eng_tokenizer,
                                               spa_tokenizer=spa_tokenizer,
                                               max_sequence_length=MAX_SEQUENCE_LENGTH,
                                               transformer=transformer)
        ...
        translated_sentence = (
            translated_sentence.replace("[PAD]", "").replace("[START]", "")
                                                    .replace("[END]", "")
                                                    .strip())
